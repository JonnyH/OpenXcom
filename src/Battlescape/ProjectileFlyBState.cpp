...
void ProjectileFlyBState::init()
{
	...
	else if (!_action.weapon->getRules()->getArcingShot())
	{
		...
		{
			// target nothing, targets the middle of the tile, but still try (to highlight obstacles)
			if (!_parent->getTileEngine()->canTargetTile(&originVoxel, targetTile, 5, &_targetVoxel, _unit, isPlayer)) //emptiness
			{
				_targetVoxel = Position(_action.target.x*16 + 8, _action.target.y*16 + 8, _action.target.z*24 + 12);
			}
		}
